...
declare variable $frus:bucket := $hsg-config:S3_BUCKET;
...
declare function frus:exists-pdf($volumeid as xs:string) as xs:boolean {
    if ($frus:STATIC-FILE-LOCATION = 'local') then
        ...
        let $collection := '/db/history/data/s3-resources/' || $frus:bucket || '/frus/'
...
declare function frus:volumes-with-ebooks() {
    for $hit in collection('/db/history/data/s3-resources/' || $frus:bucket || '/frus/')//filename[ends-with(., '.epub')]
    return
        substring-before($hit, '.epub')
};

declare function frus:volumes-with-single-pdfs() {
    for $hit in collection('/db/history/data/s3-resources/' || $frus:bucket || '/frus/')//filename[ends-with(.,'.pdf')][starts-with(., 'frus')]
    return
...
declare function frus:exists-ebook($volumeid as xs:string) as xs:boolean {
    if ($frus:STATIC-FILE-LOCATION = ('local', 'hsg')) then
        ...
        let $collection := concat('/db/history/data/s3-resources/' || $frus:bucket || '/frus/', $volumeid, '/ebook')
...
declare function frus:epub-size($volumeid as xs:string) {
    let $epub := (doc(concat('/db/history/data/s3-resources/' || $frus:bucket || '/frus/', $volumeid, '/ebook/resources.xml'))//filename[ends-with(., '.epub')]/parent::resource)[1]
    let $size := $epub//size
    return
...
declare function frus:mobi-size($volumeid as xs:string) {
    let $mobi := (doc(concat('/db/history/data/s3-resources/' || $frus:bucket || '/frus/', $volumeid, '/ebook/resources.xml'))//filename[ends-with(., '.mobi')]/parent::resource)[1]
    let $size := $mobi//size
    return
        frus:bytes-to-readable($size)
};
...
declare function frus:exists-doc-pdf($volumeid, $document) as xs:boolean {
    if ($frus:STATIC-FILE-LOCATION = ('local', 'hsg')) then
        ...
        collection(concat('/db/history/data/s3-resources/' || $frus:bucket || '/frus/', $volumeid, '/pdf/'))//filename = concat($document, '.pdf')
};
...
declare function frus:pdf-size($volumeid as xs:string) {
    if ($frus:STATIC-FILE-LOCATION = 'local') then
        ...
        frus:bytes-to-readable(collection('/db/history/data/s3-resources/' || $frus:bucket || '/frus/')//filename[. eq frus:pdf-filename($volumeid)]/following-sibling::size)
};
